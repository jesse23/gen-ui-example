...
function ReactGenComponent({ prompt }: ReactGenComponentProps) {
      ...
      generate(prompt)
        .then(async (moduleCode) => {
          ...
          for (const name of componentNames) {
            // Check if this is still the active run during async operations
            if (activeRunRef.current !== currentRun) return
          ...
          try {
            const GeneratedComponent = await loadComponentFromBlob(moduleCode, React, componentMap)
  ...
  // Always show code editor if we have code, even if there's an error
  const hasCode = code.length > 0

  if (error && !hasCode) {
    // Only show error-only view if we don't have code to show
    return (
      <div className="flex items-center justify-center h-full">
